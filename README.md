...
```
### generate translate template
put yang model with schema-mount in dir test-translate. -- mapping_rules: maping_rules.json(maping_rules) 

```
cd mdk
mkdir test-translate
cd test-translate
$ export PYBINDPLUGIN=`/usr/bin/env python3 -c \
'import pyangbind; import os; print ("{}/plugin".format(os.path.dirname(pyangbind.__file__)))'`
$pyang --plugindir $PYBINDPLUGIN -f pybind --mapping-rules mapping_rules.json -o binding.py ietf-interfaces.yang ietf-ip\@2018-02-22.yang
```
